...
1. 分支的本质就是一个提交对象
1. HEAD : 是一个指针，它默认指向master分支，切换分支时就是让HEAD指向不同的分支，每次有新的提交时，HEAD就会带着当前的分支往前移动（HEAD移动，分支不会移动）。
1. `git/refs`目录中保存了所有的分支，及其对应的提交对象的hash值（每次提交对象被新提交时，hash值都会改变）
1. 切换分支会改变workspace中的文件。
...
7. `git clone 远程仓url` ：将远程仓库拉到本地
    - `git clone`下来的远程仓库的别名，自动设置成origin
...
1. 可以通过`fork`别人的项目，修别人的bug，给别人提request。（此步骤可以用github操作，不需要命令行）
1. 一般只有大神才会用到。。
